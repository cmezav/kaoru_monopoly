...
function releaseCenterLock(token){if(token!==centerSequence)return;centerLocked=false;centerLockText='';if(current?.state)renderTokenSnapshot(current.state.players);if($('turnLabel'))$('turnLabel').textContent=turnText();updateOccupiedTiles();renderActions();renderProperty()}
