...
const cardLabels={
  chance:['Jax · Dado escondido','Caine · Cambio de escenario','Luz · Portal a GO','Trampa con el dado · Cárcel','Cámara · Salida de la cárcel','Eda · Compra basura','I.M.P. · Factura','Piltover · Ventana rota','Último tren · Estación anterior','Atajo · Avanza 2'],
  community:['Banco · Cobro duplicado','Chaqueta vieja · Dinero encontrado','Cumpleaños · Cada rival te entrega 50','Entradas para el grupo · Paga a las rivales','Concurso de cosplay · Premio','Pedido incompleto · Devolución','Multa atrasada','Tubería rota · Reparaciones','Cupón · Descuento de alquiler','Persona equivocada · Cárcel']
};
...
function ensureCards(value){const cards=value.cards||{};value.cards={chance:defaultCards.chance.map((txt,i)=>({text:cards.chance?.[i]?.text||txt,image:cards.chance?.[i]?.image||null})),community:defaultCards.community.map((txt,i)=>({text:cards.community?.[i]?.text||txt,image:cards.community?.[i]?.image||null}))};const special=value.specialCards||{};value.specialCards=Object.fromEntries(Object.entries(defaultSpecialCards).map(([pos,def])=>[pos,{label:def.label,text:special[pos]?.text||def.text,image:special[pos]?.image||null}]));return value}
...
function renderCardsEditor(){
  ...
  const title=text=>{const h=document.createElement('div');h.className='card-group-title';h.innerHTML=`<h3>${text}</h3>`;wrap.append(h)};
  const add=(card,label,resetText)=>{const section=document.createElement('section');section.className='card-edit';section.innerHTML=`<h3>${label}</h3><div class="card-shell"><div class="hero">${card.image?`<img src="${card.image}" alt="">`:'<span>Sube una imagen 16:9 para esta cartilla</span>'}</div><div class="body"><em>${label}</em><p>${card.text||''}</p></div></div><label>Imagen superior (PNG, JPG o WebP)<input type="file" accept="image/png,image/jpeg,image/webp"></label><label>Texto de la cartilla<textarea maxlength="420"></textarea></label><div class="mini-actions"><button type="button">Quitar imagen</button><button type="button">Restablecer texto</button></div>`;const file=section.querySelector('input[type=file]'),ta=section.querySelector('textarea'),remove=section.querySelectorAll('button')[0],reset=section.querySelectorAll('button')[1];ta.value=card.text||'';ta.oninput=()=>{card.text=ta.value;section.querySelector('.body p').textContent=card.text;changed()};file.onchange=async()=>{try{const src=await ingestImage(file.files[0],1280,720,.9);file.value='';if(!src)return;card.image=src;section.querySelector('.hero').innerHTML=`<img src="${src}" alt="">`;changed()}catch(e){notify(e.message)}};remove.onclick=()=>{card.image=null;section.querySelector('.hero').innerHTML='<span>Sube una imagen 16:9 para esta cartilla</span>';changed()};reset.onclick=()=>{card.text=resetText;ta.value=card.text;section.querySelector('.body p').textContent=card.text;changed()};wrap.append(section)};
  title('Casillas especiales');
  for(const [pos,def] of Object.entries(defaultSpecialCards))add(board.specialCards[pos],def.label,def.text);
  title('Cartas de Suerte que existen en la partida');board.cards.chance.forEach((card,i)=>add(card,`Suerte · ${cardLabels.chance[i]}`,defaultCards.chance[i]));
  title('Cartas de Evento que existen en la partida');board.cards.community.forEach((card,i)=>add(card,`Evento · ${cardLabels.community[i]}`,defaultCards.community[i]));
}
...
function validate(b){Model.validate(b);ensureCards(b);const validCard=card=>card&&typeof card.text==='string'&&card.text.length<=420&&(card.image===null||(typeof card.image==='string'&&/^data:image\/(png|jpeg|webp);base64,[A-Za-z0-9+/=]+$/.test(card.image)&&card.image.length<12000000));for(const kind of ['chance','community']){if(!Array.isArray(b.cards[kind])||b.cards[kind].length!==10)throw new Error('Las cartillas deben tener 10 entradas por mazo.');for(const card of b.cards[kind])if(!validCard(card))throw new Error('Revisa las cartillas: texto o imagen inválidos.')}for(const [pos,card]of Object.entries(b.specialCards))if(!defaultSpecialCards[pos]||!validCard(card))throw new Error('Revisa las cartillas de casillas especiales.')}
