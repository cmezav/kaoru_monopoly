...
let selected=0,selectedSpecial=null,dirty=false,db,drag,noticeTimer,autosaveTimer,remoteAvailable=true,saving=false;const images=new Map();let revision=0;
...
function changed(){dirty=true;revision++;$('save').textContent='Guardando…';clearTimeout(autosaveTimer);autosaveTimer=setTimeout(()=>persistBoard(true).catch(()=>{}),1800)}
...
async function saveLocal(snapshot){if(!db)db=await openDB();await new Promise((resolve,reject)=>{const tx=db.transaction('boards','readwrite');tx.objectStore('boards').put(snapshot,'draft-v5');tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error)})}
async function saveRemote(snapshot){if(!remoteAvailable)return false;const response=await fetch('/api/editor/board',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({board:snapshot})});if(response.status===401){remoteAvailable=false;return false}let data={};try{data=await response.json()}catch{}if(!response.ok)throw new Error(data.error||'No se pudo guardar el tablero en tu cuenta.');return true}
async function readRemote(){try{const response=await fetch('/api/editor/board',{credentials:'same-origin'});if(response.status===401){remoteAvailable=false;return null}if(!response.ok)return null;return await response.json()}catch{return null}}
async function persistBoard(silent=false){if(saving){clearTimeout(autosaveTimer);autosaveTimer=setTimeout(()=>persistBoard(true).catch(()=>{}),900);return}validate(board);saving=true;const savedRevision=revision,snapshot=structuredClone(board);try{await saveLocal(snapshot);const online=await saveRemote(snapshot);if(savedRevision===revision){dirty=false;$('save').textContent=online?'Guardado ✓':'Guardado local ✓'}else{clearTimeout(autosaveTimer);autosaveTimer=setTimeout(()=>persistBoard(true).catch(()=>{}),900)}if(!silent)notify(online?'Guardado en tu cuenta. Las salas nuevas usarán este tablero y estas cartillas.':'Guardado en este navegador. Inicia sesión para sincronizarlo con las salas nuevas.')}finally{saving=false}}
$('save').onclick=async()=>{try{await persistBoard(false)}catch(e){$('save').textContent='Guardar cambios •';notify('No se pudo guardar: '+e.message)}};
...
(async()=>{sync();renderBoard();renderCardsEditor();const initialRevision=revision;try{db=await openDB();const [remote,local]=await Promise.all([readRemote(),readSaved()]);if(revision===initialRevision){const chosen=remote?.saved?remote.board:(local||remote?.board||globalThis.KAORU_PRELOADED||Model.initial());await hydrate(chosen);dirty=false;$('save').textContent=remote?.saved?'Guardado ✓':'Guardar cambios';if(!remote?.saved&&local&&remoteAvailable){revision++;dirty=true;await persistBoard(true)}}}catch(e){if(!dirty&&globalThis.KAORU_PRELOADED)await hydrate(globalThis.KAORU_PRELOADED);notify('No se pudo recuperar el tablero sincronizado. Se cargó la copia disponible.')}})();
